...
def riptide_dorefa_net(hparams, input_shape, num_classes, input_tensor=None, include_top=True):
    def conv_block(x, filters, kernel_size, strides=1, pool=False, pool_padding="same"):
        ...
        x = lq.riptide.BatchNormalization(
            use_shiftnorm=hparams.use_shiftnorm,
            bits=hparams.activations_k_bit,
    ...
    def fully_connected_block(x, units):
        dense_layer = lq.layers.QuantDense(
            units,
            input_quantizer=hparams.input_quantizer,
            kernel_quantizer=hparams.kernel_quantizer,
            kernel_constraint=None,
...
@registry.register_hparams(riptide_dorefa_net)
...
def DoReFaNet(
    ...
    model = riptide_dorefa_net(
        default(), input_shape, classes, input_tensor=input_tensor, include_top=include_top,
    )
